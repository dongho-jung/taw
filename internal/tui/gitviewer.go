...
func (m *GitViewer) handleKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	// Close on q or Ctrl+G
	case "q", "ctrl+g", "ctrl+shift+g":
		return m, tea.Quit
...
func (m *GitViewer) handleSearchKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	switch msg.String() {
	case "enter":
		// Confirm search
		m.searchMode = false
...
func (m *GitViewer) View() tea.View {
	...
	case gitModeAll:
		modeStr = "[3:LOG --all]"
	case gitModeDiff:
		modeStr = "[4:DIFF " + m.mainBranch + "...HEAD]"
	}
	...
	hint := gitViewerHintFull
	padding := m.width - statusWidth - gitViewerHintFullWidth
	if padding < 0 {
		hint = gitViewerHintShort
		padding = m.width - statusWidth - gitViewerHintShortWidth
		if padding < 0 {
			padding = 0
		}
	}
...
func (m *GitViewer) scrollToMatch(idx int) {
