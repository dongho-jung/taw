...
import (
	...
	"github.com/dongho-jung/paw/internal/tui/textarea/internal/memoization"
	"github.com/dongho-jung/paw/internal/tui/textarea/internal/runeutil"
...
func (p cursorPos) after(other cursorPos) bool {
...
type Model struct {
	...
	// selectionActive indicates whether a selection is active.
...
func DefaultStyles(isDark bool) Styles {
	...
	s.Focused = StyleState{
		...
		Selection:        lipgloss.NewStyle().Background(lightDark(lipgloss.Color("153"), lipgloss.Color("57"))).Foreground(lightDark(lipgloss.Color("0"), lipgloss.Color("255"))),
	}
...
func (m *Model) SetSelection(startRow, startCol, endRow, endCol int) {
	start := m.clampSelectionPos(cursorPos{row: startRow, col: startCol})
	end := m.clampSelectionPos(cursorPos{row: endRow, col: endCol})
...
func (m Model) SelectedText() string {
	...
	b.WriteString(string(m.value[start.row][start.col:]))
	b.WriteByte('\n')
	for i := start.row + 1; i < end.row; i++ {
		b.WriteString(string(m.value[i]))
		b.WriteByte('\n')
	}
	b.WriteString(string(m.value[end.row][:end.col]))
	return b.String()
}
...
func (m Model) CopySelection() error {
	...
	return clipboard.WriteAll(m.SelectedText())
}

// DeleteSelection removes the selected text and places the cursor at the start.
...
func (m Model) clampSelectionPos(pos cursorPos) cursorPos {
	if len(m.value) == 0 {
		return cursorPos{}
	}

	row := clamp(pos.row, 0, len(m.value)-1)
	col := clamp(pos.col, 0, len(m.value[row]))
	return cursorPos{row: row, col: col}
}

func (m Model) selectionBounds() (cursorPos, cursorPos, bool) {
	if !m.selectionActive {
		return cursorPos{}, cursorPos{}, false
	}

	start := m.clampSelectionPos(m.selectionStart)
	end := m.clampSelectionPos(m.selectionEnd)
	if start == end {
		return cursorPos{}, cursorPos{}, false
	}
	if start.after(end) {
...
func (m Model) selectionOffsetsForLine(lineIndex int, line []rune, start cursorPos, end cursorPos, hasSelection bool) (int, int, bool) {
	if !hasSelection || lineIndex < start.row || lineIndex > end.row {
		return 0, 0, false
	}
	...
	startCol := 0
	endCol := lineLen
	if lineIndex == start.row {
		startCol = clamp(start.col, 0, lineLen)
	}
	if lineIndex == end.row {
		endCol = clamp(end.col, 0, lineLen)
	}
	if startCol == endCol {
		return 0, 0, false
	}
...
func (m *Model) deleteSelection() bool {
	...
	if start.row == end.row {
		m.value[start.row] = append(m.value[start.row][:start.col], m.value[start.row][end.col:]...)
	} else {
...
func (m Model) View() string {
	...
	for l, line := range m.value {
		...
		for wl, wrappedLine := range wrappedLines {
			...
			wrapEnd := wrapStart + len(wrappedLine)
			selStartInLine := 0
			selEndInLine := 0
			hasSelSegment := false
			if lineHasSelection {
				segmentStart := max(lineSelStart, wrapStart)
				segmentEnd := min(lineSelEnd, wrapEnd)
				if segmentStart < segmentEnd {
					selStartInLine = segmentStart - wrapStart
					selEndInLine = segmentEnd - wrapStart
					hasSelSegment = true
				}
			}
