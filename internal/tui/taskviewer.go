...
const (
	taskViewerHintFull       = "↑↓j/k:scroll g/G:top/end q/Esc:close"
	taskViewerHintShort      = "q:close"
	taskViewerHintFullWidth  = 36 // len("↑↓j/k:scroll g/G:top/end q/Esc:close")
	taskViewerHintShortWidth = 7  // len("q:close")
)

// TaskViewer provides an interactive task content viewer with vim-like navigation.
...
// NewTaskViewer creates a new task viewer with the given content.
func NewTaskViewer(content string) *TaskViewer {
...
// Init initializes the task viewer.
func (m *TaskViewer) Init() tea.Cmd {
	return tea.RequestBackgroundColor
}

// Update handles messages and updates the model.
func (m *TaskViewer) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	switch msg := msg.(type) {
...
func (m *TaskViewer) handleKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	switch msg.String() {
...
func (m *TaskViewer) scrollUp(n int) {
	m.scrollPos -= n
	if m.scrollPos < 0 {
		m.scrollPos = 0
	}
...
func (m *TaskViewer) scrollDown(n int) {
...
// View renders the task viewer.
func (m *TaskViewer) View() tea.View {
	...
	var status string
	if len(m.lines) > 0 {
		status = " Task " + strconv.Itoa(m.scrollPos+1) + "-" + strconv.Itoa(endPos) + " of " + strconv.Itoa(len(m.lines)) + " lines "
	} else {
		status = " (empty) "
	}

	// Keybindings hint (use pre-computed widths to avoid ansi.StringWidth on each render)
	hint := taskViewerHintFull
	padding := m.width - len(status) - taskViewerHintFullWidth
	if padding < 0 {
		hint = taskViewerHintShort
		padding = m.width - len(status) - taskViewerHintShortWidth
		if padding < 0 {
			padding = 0
		}
	}
...
func (m *TaskViewer) contentHeight() int {
...
func (m *TaskViewer) getSelectionRange() (minY, maxY, startX, endX int) {
...
func (m *TaskViewer) getSelectionXRange(screenY int) (int, int) {
...
func (m *TaskViewer) applySelectionToLine(line string, screenY int, highlightStyle lipgloss.Style) string {
...
func (m *TaskViewer) copySelection() {
	if !m.hasSelection {
		return
	}
...
// RunTaskViewer runs the task viewer with the given content.
func RunTaskViewer(content string) error {
	m := NewTaskViewer(content)
	p := tea.NewProgram(m)
	_, err := p.Run()
	return err
}
